...
def normalize_address(text: str) -> str:
    ...
    # Premise/floor/room data belongs to delivery instructions, not the map
    # search address. Keep only the building (and corpus/structure) portion.
    s = re.sub(
        r",?\s*(?:кв\.?\s*пом|пом(?:ещение)?|этаж|ком(?:ната)?\b).*?$",
        "",
        s,
        flags=re.I,
    )
